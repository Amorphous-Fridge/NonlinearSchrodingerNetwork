...
for i3 in nni[numLayers//2-1:N]:
    for i4 in nni[numLayers//2-1:N]:
        for i2 in range(0,i3):
            for i1 in range(0,i2):
                for i5 in range(0,i4):
                    for i6 in range(0,i5):                    
                        if (abs(i2-i5)>1)or(abs(i1-i6)>1)or\
                            (abs(i3-i4)>0)or\
                            ((abs(i2-i5)>0)and abs(i1-i6)>0):
                            continue
                        widths.append((nn[i1],nn[i2],nn[i3],nn[i4],nn[i5],nn[i6])) #TODO:change num inputs

(width1,width2,width3,width4,width5,width6) = widths[modelnum] #TODO: change # outputs
...
encoding_layer_6 = tf.keras.layers.Dense(width6, activation="selu", name='encoding_layer_6')(encoding_layer_5)
encoded_state = tf.keras.layers.Dense(ANTIKOOPMAN_DIMENSION, activation="selu", name='bottleneck')(encoding_layer_6)
#####################################################################################################################

#########################################DECODER#####################################################################
decoding_layer_1 = tf.keras.layers.Dense(width6, activation = "selu", name='decoding_layer_1')(antikoop_state)
decoding_layer_2 = tf.keras.layers.Dense(width5, activation = "selu", name='decoding_layer_2')(decoding_layer_1)
decoding_layer_3 = tf.keras.layers.Dense(width4, activation = "selu", name='decoding_layer_3')(decoding_layer_2)
decoding_layer_4 = tf.keras.layers.Dense(width3, activation = "selu", name='decoding_layer_4')(decoding_layer_3)
decoding_layer_5 = tf.keras.layers.Dense(width2, activation = "selu", name='decoding_layer_5')(decoding_layer_4)
decoding_layer_6 = tf.keras.layers.Dense(width1, activation = "selu", name='decoding_layer_6')(decoding_layer_5)
decoded_state = tf.keras.layers.Dense(STATE_DIMENSION, activation = "selu", name='decoded_layer')(decoding_layer_6)
#####################################################################################################################
#Model declarations
...
#SAVE TO A FILE
write_history(history, [Autoencoder, Phi, Phi_inv], datadir='./Results6L/', batch_size='4096',modelnum=str(modelnum)) #TODO:change datadir
