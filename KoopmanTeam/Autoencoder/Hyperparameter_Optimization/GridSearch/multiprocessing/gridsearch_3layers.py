...
def trainModel(width1,width2,width3):
    print('Conner is a nerd')
    ##########################################ENCODER####################################################################
    ...
    #SAVE TO A FILE
    write_history(history, [Autoencoder, Phi, Phi_inv], datadir='./Results3L/', batch_size='4096')
...
#    numCores = multiprocessing.cpu_count()
    ...
    pool.starmap(trainModel,widths)
    pool.close()
    pool.join()
#results = Parallel(n_jobs=num_cores)(delayed(processInput)(i) for i in widths)
