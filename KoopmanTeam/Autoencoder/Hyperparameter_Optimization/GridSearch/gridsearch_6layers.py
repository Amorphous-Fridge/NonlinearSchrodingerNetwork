...
N = len(nn)
nni = np.arange(0,7)
n = 0 #for counting iterations and tracking progress

for i3 in nni[numLayers//2:N]:
    for i4 in nni[numLayers//2:N]:
            ...
            for i1 in range(0,i2):
                for i5 in range(0,i4):
                    for i6 in range(0,i5):
                        ...
                        #COMPILE AND TRAIN
                        Autoencoder.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=.001),\
                                            loss=L2_loss,metrics=['mse','mae'],run_eagerly=False)
                        history = Autoencoder.fit(generate_pure_bloch(4096), steps_per_epoch=50,epochs=100)
                                            #remove generate_pure_bloch and replace with dataset
